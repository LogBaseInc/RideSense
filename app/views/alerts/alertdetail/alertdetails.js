define(['angular',
    ...
    (function () {
        ...
        configroute.register.controller('alertdetail', ['$rootScope', '$scope', 'config', 'spinner', 'uiGmapIsReady', 'uiGmapGoogleMapApi', 'sessionservice', alertdetail]);
        function alertdetail($rootScope, $scope, config, spinner, uiGmapIsReady, uiGmapGoogleMapApi, sessionservice) {
			...
			vm.locationsearch = true;
		  	...
		  	vm.dragMarker = false;
		  	vm.map = { center: { latitude: 11.012897, longitude: 76.986136 }, zoom: 2 };
		  	vm.clusterOptions = {
							       averageCenter : true,
							       ignoreHidden : true,
								};
				...
				/*clearRoutes();
			  	...
			  	for(var i=0 ; i<vm.cars.models.length; i++) {
			  		if(vm.cars.models[i].isIdle === true) {
				  		...
				  		var request = {
						    origin: "Singanallur, Coimbatore, Tamil Nadu, India", 
		           			destination: "Tatabad, Coimbatore, Tamil Nadu, India",
						    travelMode: google.maps.TravelMode.DRIVING
			  			};

					  	directionsService.route(request, function(response, status) {
						    if (status == google.maps.DirectionsStatus.OK) {
						    	var directionsDisplay = new google.maps.DirectionsRenderer({preserveViewprot: true, suppressMarkers: true});
    		...
    		models: [
	    		{
			    	...
			    	options: {
			    		...
			    		icon: 'assets/images/car-red.png'
			    	}
			    },
			    {
	    			latitude: 11.016583,
			    ...
			    },
			     {
			    	...
			    	id: 3,
			    	options: {
			    		labelContent: 'speeding on intersection', 
			    		labelClass: 'tm-marker-label',
			    		icon: 'assets/images/alert.png'
			    	}
			    }
